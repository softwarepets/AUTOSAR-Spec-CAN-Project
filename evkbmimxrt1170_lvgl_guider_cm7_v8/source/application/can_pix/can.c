...
#include "board.h"
#include "can.h"
#include "../../../Mcal/fsl_flexcan.h"
#include "../../../Services/freertos-kernel/include/FreeRTOS.h"
#include "../../../Services/freertos-kernel/include/task.h"
#include "../../../Ecual/autosar_M/Can.h"
#include "../../../Ecual/Comm/hal_flexcan.h"
#include "../../../Ecual/autosar_IF/CanIF.h"
...
//      FlexCan.config.clkSrc                 = kFLEXCAN_ClkSrc0;
//      FlexCan.config.bitRate               = 1000000U;
//      FlexCan.config.bitRateFD             = 2000000U;
//      FlexCan.config.maxMbNum               = 16;
//      FlexCan.config.enableLoopBack         = false;
//      FlexCan.config.enableSelfWakeup       = false;
//      FlexCan.config.enableIndividMask      = false;
//      FlexCan.config.disableSelfReception   = false;
//      FlexCan.config.enableListenOnlyMode   = false;
//      FlexCan.config.enableDoze             = false;

   // FLEXCAN_GetDefaultConfig(&FlexCan.config);

  //  FlexCan.config.bitRate = 500000U;

//#if (defined(USE_IMPROVED_TIMING_CONFIG) && USE_IMPROVED_TIMING_CONFIG)
//    flexcan_timing_config_t timing_config;
//    memset(&timing_config, 0, sizeof(flexcan_timing_config_t));
//    if (FLEXCAN_CalculateImprovedTimingValues(EXAMPLE_CAN, FlexCan.config.bitRate, EXAMPLE_CAN_CLK_FREQ, &timing_config))
//    {
//        /* Update the improved timing configuration*/
//        memcpy(&(FlexCan.config.timingConfig), &timing_config, sizeof(flexcan_timing_config_t));
//    }
//    else
//    {
...
//#endif
   ...
   // FLEXCAN_Init(EXAMPLE_CAN, &FlexCan.config, EXAMPLE_CAN_CLK_FREQ);
    Can_Init(&CanConfig);
   // HAL_FLEXCAN_Init(&FlexCan);
    CanIf_Init(&canIfConfig);

    /* Create FlexCAN handle structure and set call back function. */
    //FLEXCAN_TransferCreateHandle(EXAMPLE_CAN, &flexcanHandle, flexcan_callback, NULL);  //for normal MCAL driver calling
   // FLEXCAN_TransferCreateHandle(EXAMPLE_CAN, &FlexCan.handle, flexcan_callback, NULL); //for HAL abstraction level calling
    FLEXCAN_TransferCreateHandle(EXAMPLE_CAN, &CanConfig.dev.handle, flexcan_callback, NULL);//for CAN driver level calling
    ...
    for (;;)
    {
       ...
       // (void)HAL_FLEXCAN_Receive(&FlexCan);
